...
## Credit
[drannex42](https://github.com/drannex42/svelte-capacitor)
...
```bash
git clone "https://github.com/prokawsar/svelte-ionic-starter.git"
cd svelte-ionic-starter
npm install
```
...
If you're only targeting Android, you don't need to change anything, already configured for android local dev, Already have set `server.url` section in `capacitor.config.json` with `http://10.0.2.2:5001`, since [Android Studio already adds a `localhost` proxy](https://stackoverflow.com/questions/9808560/why-do-we-use-10-0-2-2-to-connect-to-local-web-server-instead-of-using-computer). Just remember to remove it when building your app for production.
...
This template was created by [t. Macleod Sawyer](https://macleodsawyer.com/) of [DNX Robotics](https://dnxrobotics.com) extended with Ionic by [Kawsar Ahmed](https://github.com/prokawsar) 